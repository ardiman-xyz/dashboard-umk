...
interface DepartmentAgeChartProps {
    ageDistribution: AgeData[];
    facultyName: string;
    onAgeClick?: (age: string) => void; // Add age click handler
}
...
export default function DepartmentAgeChart({ ageDistribution, facultyName, onAgeClick }: DepartmentAgeChartProps) {
    ...
    const handleBarClick = (data: any) => {
        if (data && data.age) {
            ...
            router.visit(url.toString(), {
                ...
                replace: true,
                onSuccess: () => {
                    // Call the onAgeClick callback if provided
                    if (onAgeClick) {
    ...
    return (
        <Card className="transition-shadow hover:shadow-lg">
            ...
            <CardFooter className="flex-col items-start gap-2 text-sm">
                ...
                <div className="text-xs text-blue-600 italic">Klik pada chart untuk melihat detail mahasiswa berdasarkan umur</div>
            </CardFooter>
        </Card>
    );
}
